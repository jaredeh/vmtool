...
build {
  ...
  # use_proxy = false: talk to the qemu port-forward directly; packer's SSH
  # proxy breaks scp/sftp with current OpenSSH. Password auth needs sshpass.
  provisioner "ansible" {
    playbook_file = "${path.root}/../../../ansible/playbooks/install_gui.yml"
    user          = var.username
    use_proxy     = false
    extra_arguments = [
